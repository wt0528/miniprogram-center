...
export default class DemandController{
    ...
    static createDemand(req, res, next){
        let param = req.query;
        DemandModel.find({"gitNum": param.gitNum}).exec((err, result) => {
            if(err){
                res.json({"code":"102","msg":"查询失败创建失败"});
    ...
    static deleteDemand(req, res, next){
        ...
        DemandModel.remove({"gitNum":param.selectedDemand}).exec((err) => {
            if(err){
                return res.json({"code":"102","msg":"删除失败"});
            } else{
                ...
                newLog.save((err) =>{
                  ...
                  if(err){
                    return res.json({"code":"101","msg":"删除成功,插入日志失败"});
                  } else{
                    return res.json({"code":"0","msg":"删除成功,插入日志成功"});
                  }
                })
            }
        })
    ...
    static getActiveDemand(req, res, next){
        ...
        DemandModel.find(findOption).exec((err, demands) =>{
            if(err){
                return res.json({"code":"101","msg":"查询失败",data:err});
            } else{
                return res.json({"code":"0","msg":"success",data:demands});
            }
